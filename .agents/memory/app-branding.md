...
7. Keep the server-managed theme preset as the installation-wide baseline. UI Lab experiments for control shape, background, cards, typography, and density are device-local and must be explicitly applied; use Google Fonts with system fallbacks so offline use remains legible.
