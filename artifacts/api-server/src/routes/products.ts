...
router.get("/", async (req, res) => {
  try {
    ...
    conditions.push(eq(productsTable.storeId, storeId), eq(productsTable.active, true));
...
router.post("/", async (req, res) => {
  try {
    ...
    const openingStock = body.stock ?? 0;
    ...
    if (openingStock > 0) {
      await db.insert(inventoryMovementsTable).values({
        ...
        idempotencyKey: `product-${product.id}-opening`,
        referenceType: "product",
        referenceId: String(product.id),
      });
      const [withStock] = await db.select().from(productsTable).where(and(eq(productsTable.id, product.id), eq(productsTable.storeId, storeId)));
      return res.status(201).json({ ...withStock, price: parseFloat(withStock.price), cost: withStock.cost ? parseFloat(withStock.cost) : null, createdAt: withStock.createdAt.toISOString() });
    }
...
router.patch("/:id", async (req, res) => {
  try {
    ...
    if (body.stock !== undefined) throw new HttpError(409, "Stock is ledger-controlled. Use an inventory adjustment instead.", "STOCK_LEDGER_REQUIRED");
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    const id = Number(req.params.id);
    if (!Number.isSafeInteger(id) || id < 1) throw new HttpError(400, "Invalid product id.");
    const [archived] = await db.update(productsTable).set({ active: false, archivedAt: new Date() }).where(and(eq(productsTable.id, id), eq(productsTable.storeId, storeId))).returning();
