...
export function StoreAdmin() {
  ...
  const [provinceCode, setProvinceCode] = useState<ProvinceCode>("ON");
  ...
  function saveStore(store: typeof stores[number]) {
    update.mutate({ id: store.id, data: { name: editName.trim() || store.name, address: store.address, phone: store.phone, email: store.email, isDefault: store.isDefault, active: store.active, provinceCode: store.provinceCode, currency: "CAD" } }, {
      ...
      <CardContent className="space-y-4">
        <p className="text-xs text-muted-foreground">Locations scope lists, reports, numbering, and new records. Existing single-store records remain available when no location is selected.</p>
         <div className="grid grid-cols-2 gap-3 rounded-md border border-border p-3">
          ...
          <div className="space-y-1.5"><Label>Province / territory</Label><Select value={provinceCode} onValueChange={(value) => setProvinceCode(value as ProvinceCode)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{provinces.map((code) => <SelectItem key={code} value={code}>{code}</SelectItem>)}</SelectContent></Select></div>
        ...
        <div className="space-y-2">
          {stores.map((store) => <div key={store.id} className="rounded-md border border-border p-3 space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs text-muted-foreground">
              <span>{store.address || "No address"}</span><span>{store.provinceCode} · {store.currency}</span>
            </div>
            <div className="flex gap-2">
              ...
              <Button type="button" variant="outline" size="sm" className="gap-1" onClick={() => update.mutate({ id: store.id, data: { name: store.name, address: store.address, phone: store.phone, email: store.email, isDefault: !store.isDefault, active: store.active, provinceCode: store.provinceCode, currency: "CAD" } }, { onSuccess: refresh })}><Power className="h-3 w-3" /> {store.isDefault ? "Default" : "Make default"}</Button>
              <Button type="button" variant="ghost" size="sm" onClick={() => update.mutate({ id: store.id, data: { name: store.name, address: store.address, phone: store.phone, email: store.email, isDefault: store.isDefault, active: !store.active, provinceCode: store.provinceCode, currency: "CAD" } }, { onSuccess: refresh })}>{store.active ? "Disable" : "Enable"}</Button>
            </div>
          </div>)}
